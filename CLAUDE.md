...
- `T-###` numbering is creation order across **all** trajectories, archived included, so archiving never renumbers its neighbours — the number lands in export filenames. VAPORIZE is the exception: a hard delete **does** renumber everything after it, so previously exported files keep a T-ID that no longer matches the app.
- Trajectories have three levels of removal: ABANDON is a status, ARCHIVE is the soft delete (`is_active = false`), and VAPORIZE is the only true hard delete in the app. Vaporize clears `trajectory_log`, `trajectory_tags`, and `exports_log` stamps, nulls `tasks.trajectory_id` (directives survive), then deletes the row — in that order, since the business FKs have no `ON DELETE CASCADE`. `D-###` for attached directives follows the directives manifest (active tasks, creation order).
